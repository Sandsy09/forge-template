...
rendering. The direct Copier path does not consume ProjectSpec. Released
`create-forge` constructs protocol `1` requests behind `new --engine-preview`
and declares the compatible `forge-template>=0.4.1,<0.5` engine range; the
released default Copier path remains separate.
