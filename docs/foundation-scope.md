...
| Coverage reporting, pre-commit feedback, documentation, changelog support, dependency-update automation, configuration examples, and editor-specific integration | **Capabilities**. `jupyter` and `scientific-python` are shipped; FT-15.03 / [ADR 0060](adr/0060-platform-composition-and-generated-tooling.md) assigns the rest to eight named capabilities (`coverage`, `pre-commit`, `pyright`, `changelog`, `documentation`, `dotenv-example`, `dependabot`, `renovate`), which [FT-17.03](https://github.com/Sandsy09/forge-template/issues/152) implements. The Forge default profile stays editor-neutral. |
