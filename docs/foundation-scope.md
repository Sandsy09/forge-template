...
capabilities -- plus, on `main`, the `github` platform (FT-17.02 / ADR 0063,
not yet released), each composing over the same one Foundation source. The
...
as the current working directory or a discovered project root. Runtime owners
that raise their own failures follow the canonical
[exception ownership conventions](exception-ownership.md), which keep
exception types owner-local, require a failure to be handled once, and assign
the translation of an escaped failure into a process outcome to the runtime
entrypoint.
...
| A root Streamlit launcher, a typed package holding the starter page, the Streamlit runtime dependency, its usage-statistics-off configuration, a `run` task and an in-process app test | **Streamlit archetype**, defined by the [accepted contract](streamlit-archetype.md) (FT-19.01 / [ADR 0068](adr/0068-streamlit-project-shape.md)) and **not yet in the catalogue**; FT-20.01 implements it. Deployment, authentication, databases and plugin ecosystems are excluded. |
| Coverage reporting, pre-commit feedback, documentation, changelog support, dependency-update automation, configuration examples, and editor-specific integration | **Capabilities**. `jupyter` and `scientific-python` shipped first; FT-15.03 / [ADR 0060](adr/0060-platform-composition-and-generated-tooling.md) assigned the rest to eight named capabilities (`coverage`, `pre-commit`, `pyright`, `changelog`, `documentation`, `dotenv-example`, `dependabot`, `renovate`), which [FT-17.03](https://github.com/Sandsy09/forge-template/issues/152) / [ADR 0064](adr/0064-implement-approved-generated-content-parity.md) shipped. The Forge default profile stays editor-neutral. |
...
| GitHub Actions, issue and pull-request templates, CODEOWNERS, and other GitHub-specific adapters | **GitHub platform** contributions. FT-15.03 / [ADR 0060](adr/0060-platform-composition-and-generated-tooling.md) fixed one `github` platform with a single required `organisation` option (repository URL and CODEOWNERS handle derived from it), owning the CI workflow, CODEOWNERS, and issue/PR templates; [FT-17.02](https://github.com/Sandsy09/forge-template/issues/151) / [ADR 0063](adr/0063-implement-the-github-platform.md) shipped it. Provider-specific files used by a capability are supplied through that platform integration, and remote workflow dependencies follow the [GitHub Action pinning policy](github-action-pinning.md). |
...
- the concrete runtime shape of a future component; runtime configuration,
  environment inputs, logging, path/resource access, and exception handling
  are defined by the [ownership](configuration-ownership.md),
