...
| `github_org` | `repo_url`, CODEOWNERS, MkDocs `repo_name` | gap | provider | cutover-blocking | FT-15.03 → FT-17.02 | GitHub platform descriptor renders host-scoped identity; value stays client-supplied |
| `repo_url` | README install, `[project.urls]`, CONTRIBUTING, SECURITY links | gap | provider | cutover-blocking | FT-15.03 → FT-17.02 | GitHub platform contributes host links into Foundation's neutral guidance via a reviewed extension point |
...
| `codeowners_team` | `.github/CODEOWNERS` entry | gap | provider | cutover-blocking | FT-15.03 → FT-17.02 | GitHub platform renders CODEOWNERS; team handle is a platform option |
...
| `python_matrix` | CI test matrix | gap | provider | cutover-blocking | FT-15.03 → FT-17.02 | value is delivered (`PythonSelection.tested_versions`); the CI job that consumes it is the GitHub platform gap |
...
| Behaviour | Copier source | Engine status | Disposition | Tier | Implementation owner | Validation requirement |
| --- | --- | --- | --- | --- | --- | --- |
| `template/.github/ISSUE_TEMPLATE/bug_report.yml` | Bug report form | gap | provider | deferred | FT-15.03 → FT-17.02 | GitHub platform contribution |
| `template/.github/ISSUE_TEMPLATE/config.yml` | Issue chooser config | gap | provider | deferred | FT-15.03 → FT-17.02 | GitHub platform contribution |
...
| Behaviour | Copier source | Engine status | Disposition | Tier | Implementation owner | Validation requirement |
| --- | --- | --- | --- | --- | --- | --- |
| `operating-systems` | Generated CI runs `ubuntu-latest` only; generation itself runs on any OS | gap | provider | cutover-blocking | FT-15.03 → FT-17.02 | GitHub platform reproduces the Linux-only CI matrix; engine rendering stays OS-neutral, already proven by `poe combos` / `poe archetype` on Windows |
...
- **Seven unrouted questions.** `github_org`, `repo_url`, `codeowners_team`,
  ...
  capability owners.
- **No GitHub platform component.** The catalogue has three archetypes and
  two capabilities; the `platforms` tuple in `ProjectSpec` accepts selections
  but `discover_components()` returns no platform. CI, CODEOWNERS, issue and
  PR templates, and host-scoped README/`pyproject` links all wait on it.
  FT-15.03 defines it and the Foundation extension points it needs; FT-17.02
  implements it.
...
what a real `library` `render_project()` call actually produces.
