...
| A `create-forge` pinned to `forge-template>=0.4.1,<0.5` still resolves, installs and generates after `0.5.0` publishes | FT-18.01 | `uv run pytest -m cutover tests/test_released_client_compatibility.py` (hermetic; also the `released-client` CI job) | FT-18.01 |
| The integrated cutover — the immutable released provider plus the candidate client — passes the full cross-repository acceptance matrix | FT-18.01 | `uv run pytest -m cutover tests/test_released_provider_cutover.py`, evidenced in [integrated-cutover-validation.md](integrated-cutover-validation.md) | FT-18.01 |
...
| Integrated cutover validation | FT-18.01 / #156, CF-18.07 / create-forge#164 | The released provider paired with the candidate client | **Done ([ADR 0067](adr/0067-validate-the-integrated-engine-default-cutover.md))** — the full cross-repository matrix passes together against the digest-verified `0.5.0` artefacts and the candidate `0.4.0` client, evidenced in [integrated-cutover-validation.md](integrated-cutover-validation.md); no provider defect was found, so no corrected release was required. A provider defect still requires a corrected reviewed release and renewed client adoption evidence, never an in-place change to `0.5.0` |
...
| [FT-18.01](https://github.com/Sandsy09/forge-template/issues/156) | The integrated matrix rows and the corrected-release rule (here) | **Done ([ADR 0067](adr/0067-validate-the-integrated-engine-default-cutover.md))** — the released `0.5.0` wheel verified against its published digests and installed from file, paired with the candidate `create-forge 0.4.0` built from the unmodified sibling checkout; every client-observable Engine, Generated-project, Independence and Regression row re-executed against the released artefact, with the exhaustive sweep and two of three full-composition cells cited rather than repeated — [integrated-cutover-validation.md](integrated-cutover-validation.md) |
